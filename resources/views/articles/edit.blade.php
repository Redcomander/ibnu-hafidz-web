...
@section('content')
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <div class="mb-8 text-center">
                ...
                <p class="mt-2 text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
                    Update your article content and settings. Make your content even better.
                </p>
            </div>
                ...
                <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            placeholder="Enter a captivating title" value="{{ old('title', $article->title) }}" required>
                        @error('title')
                        ...
                        <div class="mt-1 flex items-center space-x-4">
                            <div class="flex-shrink-0 h-32 w-32 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-700 border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center thumbnail-preview"
                                ...
                                @if($article->thumbnail)
                                    <img src="{{ asset('storage/' . $article->thumbnail) }}" class="h-full w-full object-cover"
                    ...
                    <div>
                        ...
                        <input type="hidden" name="body" id="quill-body" value="{{ old('body', $article->body) }}">
                        @error('body')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="flex items-center space-x-3">
                            <label for="status-toggle" class="inline-flex items-center cursor-pointer">
                                <div class="relative status-toggle">
                                    ...
                                    <div
                                        class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-300 ease-in-out {{ $article->status === 'published' ? 'transform translate-x-6 bg-green-500' : '' }}">
                                    </div>
                                </div>
                            </label>
                            <span id="status-text"
                                class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ ucfirst($article->status) }}</span>
    ...
    <script>
        ...
        const content = document.getElementById('quill-body').value;
        if (content) {
